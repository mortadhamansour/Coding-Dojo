...
const pkmnnamesAndIds = pokemon.filter(p => p.id > 99).map(p => p.name);
console.log(pkmnnamesAndIds);
const pkmnnamesAndTypes = pokemon.filter(p => p.types.length === 1 && p.types[0] === "poison").map(p => p.name);
console.log(pkmnnamesAndTypes);
const pkmnnamesAndSecondTypes = pokemon.filter(p => p.types[1] === "flying").map(p => p.types[0]);
console.log(pkmnnamesAndSecondTypes);
